...
| 功能模块 | 当前版本 (v1) | 未来规划 |
|---|---|---|
| **代理拦截** | ✅ HTTP 代理，透明转发 | 支持多 API 端点 |
| **Token 统计** | ✅ 输入/输出/缓存 token | 分工具类型统计 |
| **费用计算** | ✅ 实时计算，支持所有主流模型 | 自定义汇率换算 |
| **Session 管理** | ✅ UUID 标识 session | 手动命名 session |
| **历史记录** | ✅ JSON 文件，按日期分目录 | 导出 CSV/Excel |
| **Web Dashboard** | ✅ 总览/Session/记录 三页 | 实时推送（SSE） |
| **流式响应** | ✅ SSE 透传 + 事后解析 | 实时进度展示 |
| **CLI 状态** | ✅ `cco status` 快速查看 | 终端 TUI 模式 |
| **多项目支持** | 🚧 基于 sessionId 区分 | 项目标签/分组 |
| **告警通知** | ❌ | 费用超限通知 |

### 2.2 数据模型

每次 API 调用保存为独立 JSON 文件：
...
单条记录包含：请求体、响应体、token 用量、费用明细、耗时、状态。
