...
// ─── 读取 ────────────────────────────────────────────────────────────────────
/** 读取单条记录（不知道日期时全局搜索） */
export function getRecord(id, date) {
...
/** 分页读取记录（跨日期，降序） */
export function getRecentRecords(limit = 50, offset = 0) {
    const all = [];
...
export function getSessionSummaries(days = 30) {
        ...
        for (const r of getRecordsByDate(date)) {
            ...
            if (!s) {
                sessionMap.set(r.sessionId, {
                    ...
                    stats: {
                        total_requests: 1,
                        total_input_tokens: r.usage?.input_tokens ?? 0,
                        total_output_tokens: r.usage?.output_tokens ?? 0,
                        total_cost_usd: r.cost?.total_usd ?? 0,
                        models_used: r.model ? { [r.model]: 1 } : {},
                    },
                });
            }
            else {
                if (r.timestamp > s.updated_at)
                    s.updated_at = r.timestamp;
                if (r.timestamp < s.created_at)
                    s.created_at = r.timestamp;
                s.stats.total_requests++;
                s.stats.total_input_tokens += r.usage?.input_tokens ?? 0;
                s.stats.total_output_tokens += r.usage?.output_tokens ?? 0;
                s.stats.total_cost_usd += r.cost?.total_usd ?? 0;
                if (r.model) {
                    s.stats.models_used[r.model] = (s.stats.models_used[r.model] ?? 0) + 1;
                }
            }
        }
    }
